...
module.exports = {
    run: async (message, args, client) => {
        ...
        message.channel.send(new Discord.MessageEmbed().setAuthor(`${member.user.tag} a été warn`, member.user.displayAvatarURL())
        ...
        if(client.db.warns[member.id].length === 5){
            let muteRole = message.guild.roles.cache.find(role => role.name === 'Muted')
                    if (!muteRole) {
                        muteRole = await message.guild.roles.create({
                            data: {
                    ...
                    const duration = parseDuration("15d")
                    const reason = "Trop d'infraction"
                    await member.roles.add(muteRole)
                    message.channel.send(`${member} a été mute pendant ${humanizeDuration(duration, {language: 'fr'})} !`).then(sent => sent.delete({timeout: 5e3}))
                    message.guild.channels.cache.get(config.logs).send(new Discord.MessageEmbed()
        ...
        if(client.db.warns[member.id].length === 8){
            const reason = "Trop d'infraction"
        let muteRole = message.guild.roles.cache.find(role => role.name === 'Muted')
        if (!muteRole) {
            muteRole = await message.guild.roles.create({
